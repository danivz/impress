...
static XAxiCdma DmaInstancePtr;
...
void init_virtual_architecture_PCAP() {
  init_PCAP();
  #if FINE_GRAIN
...
void init_virtual_architecture_HBICAP() {
  init_HBICAP();
  #if FINE_GRAIN
  find_number_of_fine_grain_blocks();
  init_constant_frames();
...
static int init_HBICAP() {
  ...
  status = HBICAP_Initialize(&xCAP_component, &DmaInstancePtr, PCAP_ID, CDMA_ID);
  if (status != XST_SUCCESS) {
    return XST_FAILURE;
  }
...
*/
int change_partition_element_PCAP(virtual_architecture_t *virtual_architecture, int x, int y, int element_info) {
  int status;
  pblock pblock_1;
  ...
  enable_PCAP();
  status = write_subclock_region_PBS_PCAP(&xCAP_component, (u32*) INITIAL_ADDR_RAM, filename, &pblock_1, 1, 0, 0);
...
* NOTE en la definicion de la funcion explicar que de momento no se elimina la info de las columnas y constantes pero que podría ser interesante para un futuro.
*/
int change_partition_element_HBICAP(virtual_architecture_t *virtual_architecture, int x, int y, int element_info) {
  int status;
  pblock pblock_1;
  ...
  status = write_subclock_region_PBS_HBICAP(&DmaInstancePtr, XPAR_AXI_HBICAP_0_S_AXI_CTRL_BASEADDR, XPAR_AXI_HBICAP_0_S_AXI_BASEADDR, (u32*) INITIAL_ADDR_RAM, filename, &pblock_1, 1, 0, 0);
  
  return status;
}


int change_partition_element_stacked_modules_PCAP(virtual_architecture_t *virtual_architecture, int x, int y, int element_info, u8 first_module, u8 last_module) {
  int status;
  pblock pblock_1;
  u8 stacked_modules;
  ...
  status = write_subclock_region_PBS_PCAP(&xCAP_component, (u32*) INITIAL_ADDR_RAM, filename, &pblock_1, 1, 0, stacked_modules);

  return status;
}

int change_partition_element_stacked_modules_HBICAP(virtual_architecture_t *virtual_architecture, int x, int y, int element_info, u8 first_module, u8 last_module) {
  int status;
  pblock pblock_1;
  u8 stacked_modules;
  ...
  status = write_subclock_region_PBS_HBICAP(&DmaInstancePtr, XPAR_AXI_HBICAP_0_S_AXI_CTRL_BASEADDR, XPAR_AXI_HBICAP_0_S_AXI_BASEADDR, (u32*) INITIAL_ADDR_RAM, filename, &pblock_1, 1, 0, stacked_modules);
