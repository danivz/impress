...
*
*****************************************************************************/
int write_subclock_region_PBS_PCAP(XDcfg *InstancePtr, u32 *addr_start, const char *file_name, pblock pblock_list[], u32 num_pblocks, u32 erase_bram, u8 stacked_modules) {
	...
	if (stacked_modules <= 1){
		...
		for (i = 0; i < num_pblocks; i++) {
			...
			for(y = initial_clock_region_row; y <= final_clock_region_row; y++) {
			  ...
			  for(int z = 0; z < 50; z++) xil_printf("#%d: 0x%08x\n", z, addr_start[z]);
			  if (status != XST_SUCCESS) {
				return XST_FAILURE;
			  }
			}
...
*
*****************************************************************************/
int write_subclock_region_PBS_HBICAP(XAxiCdma *DmaInstancePtr, u32 DeviceCtrlAddr, u32 DeviceMemAddr, u32 *addr_start, const char *file_name, pblock pblock_list[], u32 num_pblocks, u32 erase_bram, u8 stacked_modules) {
	...
	if (stacked_modules <= 1){
		...
		for (i = 0; i < num_pblocks; i++) {
			...
			for(y = initial_clock_region_row; y <= final_clock_region_row; y++) {
			  ...
			  //We read the actual content on the FPGA and save it on the RAM memory
			  status = HBICAP_RAM_read(DmaInstancePtr, DeviceCtrlAddr, DeviceMemAddr, &previous_PBS_last_addr, x0, y, xf, y);
			  for(int z = 0; z < 50; z++) xil_printf("#%d: 0x%08x\n", z, addr_start[z]);
			  if (status != XST_SUCCESS) {
				return XST_FAILURE;
			  }
...
//		}
	...
	if (stacked_modules == 0 || stacked_modules == 3){
		...
		for (i = 0; i < num_pblocks; i++) {
			...
			for(y = initial_clock_region_row; y <= final_clock_region_row; y++) {
			  status = HBICAP_RAM_write(DmaInstancePtr, DeviceCtrlAddr, DeviceMemAddr, pblock_addr[reconfigurable_regions], (u32) (pblock_addr[reconfigurable_regions + 1]), x0, y, xf, y, erase_bram);
			  if (status != XST_SUCCESS) {
				return XST_FAILURE;
			  }
			  reconfigurable_regions++;
